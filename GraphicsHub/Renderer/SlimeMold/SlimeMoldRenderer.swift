...
class SlimeMoldRenderer: Renderer {
    ...
    override func draw(commandBuffer: MTLCommandBuffer, view: MTKView) {
        ...
        calculateEncoder?.setBytes([Int32(inputManager.speed)], length: MemoryLayout<Int32>.stride, index: 5)
...
class SlimeMoldInputManager: BasicInputManager {
    ...
    var colors: [NSColor] {
        (getInput(4) as! ListInput<NSColor, ColorPickerInput>).output
    }
    ...
    override init(renderSpecificInputs: [NSView] = [], imageSize: CGSize?) {
        ...
        let speed = SliderInput(name: "Speed", minValue: 1, currentValue: 1, maxValue: 10, tickMarks: 10)
        let diffusionSize = SliderInput(name: "Diffusion Size", minValue: 1, currentValue: 1, maxValue: 10, tickMarks: 10)
        let diffusionRate = SliderInput(name: "Diffusion Rate", minValue: 0.01, currentValue: 0.1, maxValue: 1)
        ...
        super.init(renderSpecificInputs: [attraction, speed, diffusionSize, diffusionRate, colorList] + renderSpecificInputs, imageSize: imageSize)
    }
