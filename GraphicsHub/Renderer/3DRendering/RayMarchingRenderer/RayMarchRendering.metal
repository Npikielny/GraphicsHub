...
float BoxDistance(float3 ray, Object Cube) {
    
    float3x3 Rx = float3x3(float3(1.0,0.0,0.0),
                           float3(0.0,cos(-Cube.rotation.x),-1*sin(-Cube.rotation.x)),
                           float3(0.0,sin(-Cube.rotation.x),cos(-Cube.rotation.x)));
    float3x3 Ry = float3x3(float3(cos(-Cube.rotation.y),0,sin(-Cube.rotation.y)),
                           float3(0,1,0),
                           float3(-sin(-Cube.rotation.y),0,cos(-Cube.rotation.y)));
    float3x3 Rz = float3x3(float3(cos(-Cube.rotation.z),-sin(-Cube.rotation.z),0),
                           float3(sin(-Cube.rotation.z),cos(-Cube.rotation.z),0),
                           float3(0,0,1));
    float3x3 RotationMatrix = Rx*Ry*Rz;
    float3 rotatedPoint = ((ray-Cube.position)*RotationMatrix);
    
    float3 q = abs(rotatedPoint)-Cube.size;
    
    return length(max(q,0.0)+min(max3(q.x, q.y, q.z), 0.0));
}
...
float SDF(Ray ray, constant Object * objects, int objectCount, thread Object & object) {
...
float3 estimateNormal (float3 origin, Object object, float precision) {
    return normalize(float3(getDistance(origin + float3(precision, 0, 0), object) -  getDistance(origin - float3(precision, 0, 0), object),
                            getDistance(origin + float3(0, precision, 0), object) -  getDistance(origin - float3(0, precision, 0), object),
                            getDistance(origin + float3(0, 0, precision), object) -  getDistance(origin - float3(0, 0, precision), object)));
}
...
kernel void rayMarch (uint2 tid [[thread_position_in_grid]],
    ...
    while (iterations < maxIterations && dist < maxDistance) {
        dist = SDF(ray, objects, objectCount, object);
        if (dist < precision) {
            float3 normal = getNormal(ray.origin, object, precision);
            float3 result = saturate(dot(normal, lightingDirection.xyz) * -1) * lightingDirection.w * object.material.albedo;
